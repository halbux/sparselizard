...
// D is the distance bwteen waveguides
...
{	
	...
	std::cout<<"look around n_eff= "<<neff_guess<<"    beta= "<<bt<<" rad*m-1"<<endl<<endl;
	...
	mode_name << std::setprecision(15);
	
	//Create mode pvd file which register each the n_eff of each mode
    //-----------------------------------------------------------------------------
    std::string filename="modes.pvd";
    ofstream modes;
    modes.open(filename);
    modes << "<?xml version=\"1.0\"?>\n";
    modes << "<VTKFile type=\"Collection\" version=\"0.1\" byte_order=\"LittleEndian\">\n";
    modes << "	<Collection>\n";
    modes.close();
	...
	//Create two field to store the full electric field (Et, El)
	// We need to separate into clad and waveguide regions to keep the discontinuity
	field Eclad("h1xyz"),Ewav("h1xyz");
	Eclad.setorder(clad,2);
	Ewav.setorder(regionunion({wavg1,wavg2}),2);
    ...
    for (int i = 0; i < myrealeigenvalues.size(); i++)
    {
		if( myrealeigenvalues[i] < 0 )
        {
			...
			//reconstruct the 3D electric field E=(Et,El):
			Eclad.setvalue(clad,expression(3,1,{Et*ex,Et*ey,El}));
			Ewav.setvalue(regionunion({wavg1,wavg2}),expression(3,1,{Et*ex,Et*ey,El}));
			
			
			//compute propagation constant and effective refractive index
			btc=sqrt(abs(myrealeigenvalues[i]));
			neffc=btc/k0;
			// Write the reconstructed electric field E in a file :
			Eclad.write(clad, "Eclad_"+ std::to_string(md) +".vtu", 2);
			Ewav.write(regionunion({wavg1,wavg2}), "Ewav_"+ std::to_string(md) +".vtu", 2);
			// display retrieved mode information
			std::cout<<"beta["<<md<<"] = "<<btc<<" rad*m-1    neff= "<<neffc<<endl;
			
			//regiester the mode files in the .pvd container
			mode_name << neffc;
			modes.open(filename, std::ios_base::app);
            modes << "		<DataSet timestep=\""+mode_name.str()+"\" part=\"0\"\n";
            modes << "			file=\"Eclad_"+ std::to_string(md)+".vtu\"/>\n";
			modes << "		<DataSet timestep=\""+mode_name.str()+"\" part=\"0\"\n";
			modes << "			file=\"Ewav_"+ std::to_string(md)+".vtu\"/>\n";
            modes.close();
			mode_name.str("");
			md++;
