...
void formulation::operator+=(integration integrationobject)
{
    ...
    for (size_t slice = 0; slice < tfs.size(); slice++)
    {
        ...
        if (mycontributions[contribindex].size() < (size_t) blocknumber+1)
...
void formulation::generate(int m, int contributionnumber)
{
    ...
    if (((size_t) contributionnumber >= mycontributions[m].size()) ||
        (mycontributions[m][contributionnumber].size() == 0))
        return;
