...
densematrix::densematrix(long int numberofrows, long int numberofcolumns)
{
...
densematrix::densematrix(long int numberofrows, long int numberofcolumns, double initvalue)
{
    numrows = numberofrows;
    numcols = numberofcolumns;
...
densematrix::densematrix(long int numberofrows, long int numberofcolumns, const std::vector<double> valvec)
{
    numrows = numberofrows;
    numcols = numberofcolumns;
    double* myvaluesptr = new double[numcols*numrows];
...
densematrix::densematrix(long int numberofrows, long int numberofcolumns, double init, double step)
{
    numrows = numberofrows;
    numcols = numberofcolumns;
    double* myvaluesptr = new double[numcols*numrows];
...
densematrix::densematrix(std::vector<densematrix> input)
{
	...
	for (long int i = 0; i < input.size(); i++)
	{
		...
		for (long int j = 0; j < input[i].count(); j++)
		{
...
void densematrix::setrow(long int rownumber, std::vector<double> rowvals)
{
    double* myvaluesptr = myvalues.get();
    for (long int i = 0; i < numcols; i++)
...
void densematrix::insert(long int row, long int col, densematrix toinsert)
{
    double* myvaluesptr = myvalues.get();
    double* toinsertvaluesptr = toinsert.myvalues.get();

    for (long int i = 0; i < toinsert.numrows; i++)
    {
        for (long int j = 0; j < toinsert.numcols; j++)
...
void densematrix::insertatrows(std::vector<int> selectedrows, densematrix toinsert)
{
    ...
    for (long int i = 0; i < selectedrows.size(); i++)
    {
...
void densematrix::insertatcolumns(std::vector<int> selectedcolumns, densematrix toinsert)
{
    ...
    {
        for (long int j = 0; j < selectedcolumns.size(); j++)
            myvaluesptr[row*numcols+selectedcolumns[j]] = toinsertvaluesptr[row*toinsert.numcols+j];
    }
}

double densematrix::getvalue(long int rownumber, long int columnnumber)
{
    double* myvaluesptr = myvalues.get();
    return myvaluesptr[rownumber*numcols+columnnumber];
}

void densematrix::setvalue(long int rownumber, long int columnnumber, double val)
{
    double* myvaluesptr = myvalues.get();
...
densematrix densematrix::multiply(densematrix B)
{
    ...
    long int numrowsA, numcolsA, numrowsB, numcolsB;
    if (not(istransposed))
    {
        numrowsA = numrows;
        numcolsA = numcols;
    ...
    long int sizethreshold = 100*100;
    ...
    else
    {
        ...
        for (long int m = 0; m < numrowsA; m++)
        {
            for (long int n = 0; n < numcolsB; n++)
            {
                Cmyvaluesptr[m*numcolsB+n] = 0;
                for (long int k = 0; k < numcolsA; k++)
                {
                    double valA, valB;
                    if (not(istransposed))
...
double densematrix::max(void)
{
    ...
    for (long int i = 1; i < numrows*numcols; i++)
    {
...
densematrix densematrix::multiplyallrows(densematrix input)
{
    ...
    for (long int i = 0; i < numrows; i++)
    {
        for (long int j = 0; j < input.numrows; j++)
        {
            for (long int k = 0; k < numcols; k++)
...
densematrix densematrix::duplicatevertically(long int n)
{
    ...
    {
        long int offset = duplicate*numrows*numcols;
        for (long int i = 0; i < numrows*numcols; i++)
...
densematrix densematrix::extractcols(std::vector<int> selected)
{
	...
	for (long int i = 0; i < numrows; i++)
	{
		for (long int j = 0; j < numselected; j++)
		{
			long int col = selected[j];
...
densematrix densematrix::extractrows(long int rangebegin, long int rangeend)
{
	std::vector<int> selected(rangeend-rangebegin+1);
	for (long int i = 0; i < selected.size(); i++)
		selected[i] = rangebegin + i;
		
	return extractrows(selected);
}

densematrix densematrix::extractcols(long int rangebegin, long int rangeend)
{
	std::vector<int> selected(rangeend-rangebegin+1);
	for (long int i = 0; i < selected.size(); i++)
		selected[i] = rangebegin + i;
